...
`;
    ...
    RW.wizard = {
        open: function () {
            try {
                console.log('[环开场] open 步骤1: 关闭其他面板');
                if (HOST.Rimworld && HOST.Rimworld.closeAllPanels) HOST.Rimworld.closeAllPanels('rw-wizard');
                console.log('[环开场] open 步骤2: 建立面板 DOM');
                var panel = ensurePanel();
                panel.classList.add('open');
                console.log('[环开场] open 步骤3: 渲染内容（display=', panel.style.display || '(css)', 'class=', panel.className, '）');
                renderBody();
                if (RW.toast) RW.toast('🌍 开局向导已打开（全屏遮罩中央；若未显示请按 F12 看 Console）', 'good');
                console.log('[环开场] open 完成 ✓');
            } catch (eOpen) {
