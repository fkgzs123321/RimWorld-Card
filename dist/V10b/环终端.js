...
`;
    ...
    function bindMapTab() {
        ...
        RW.map.render();
        if (RW.map.attach && !canvas.__rwAttached) { canvas.__rwAttached = true; RW.map.attach(canvas); } // 完整版：点击情报卡/派人/缩放/侦察
    ...
    // Tab：殖民者
    var SKILL_NAMES = ['射击', '格斗', '建造', '采矿', '烹饪', '种植', '畜牧', '手工', '艺术', '医疗', '社交', '智识']; // 与 schema 技能键一致
    function renderColonistTab(st) {
        var names = ['薇卡', '凯奥', '小满'];
        var portraits = { '薇卡': '👩‍⚕️', '凯奥': '👨‍🌾', '小满': '🔧' };
        var stages = { '薇卡': ['观察对象', '参照标准', '唯一变量'], '凯奥': ['礼貌协作', '兄弟', '无条件托付'], '小满': ['讨原型', '信任', '黏人占有欲'] };
        var html = '';
        for (var i = 0; i < names.length; i++) {
            ...
            var traits = Array.isArray(p.特质) ? p.特质 : String(p.特质 || '').split('、').filter(Boolean); // 特质为、分隔字符串（zod 铁律）
    ...
    function buildPanel() {
        ...
        root.addEventListener('click', function (e) {
            ...
            // 乐观更新：立即翻转 UI（写回失败再弹回）
            sw.classList.toggle('on', !wasOn);
            var ok = false, errMsg = '';
            try {
                ok = writeState(function (s) {
                    s.模组 = s.模组 || {}; s.模组.已安装 = s.模组.已安装 || {};
                    s.模组.已安装[id] = !wasOn ? 1 : 0;
                }, '模组开关：' + id);
                if (!ok) errMsg = 'MVU 未就绪（请先发送一条消息初始化）';
            } catch (eW) { errMsg = eW && eW.message || '未知错误'; }
            if (!ok) {
                sw.classList.toggle('on', wasOn); // 弹回
                alert('[环世界] 模组「' + id + '」状态写入失败：' + errMsg);
            } else {
                // 读回校验：防 zod 静默剥除（写入成功但读回旧值=键/枚举不匹配）
                var verify = false;
                try {
                    var m2 = (window.Mvu || HOST.Mvu);
                    var r2 = m2 && m2.getMvuData && m2.getMvuData();
                    verify = !!(r2 && r2.stat_data && r2.stat_data.模组 && r2.stat_data.模组.已安装 && r2.stat_data.模组.已安装[id] === (!wasOn ? 1 : 0));
                } catch (eV) {}
                if (verify) { toast(id + ' 已' + (!wasOn ? '启用' : '停用') + '（对应内容' + (!wasOn ? '开始' : '不再') + '进入 AI 上下文）', 'good'); return; } // 乐观 UI 已生效：不重渲染（MVU 立即读回可能滞后导致开关被旧值覆盖）
                else alert('[环世界] 模组「' + id + '」写入后读回不一致（可能被 zod 校验剥除）。请把此弹窗截图发给作者。');
            }
            renderCurrent();
        });
    ...
    function bindQuickbar(root) {
        ...
        qb.innerHTML = '<button class="rw-btn primary" id="rw-advance">⏩ 推进半天</button>' +
            ...
            '<button class="rw-btn" id="rw-qb-codex">📖 百科</button>' +
            '<button class="rw-btn" id="rw-qb-workshop">🔧 工坊</button>' +
            '<button class="rw-btn" id="rw-qb-wizard">🌍 开局</button>' +
            '<button class="rw-btn" id="rw-qb-craft">⚗️ 制造</button>' +
            '<button class="rw-btn" id="rw-caravan">🐴 远行</button>' +
            '<button class="rw-btn" id="rw-qb-farm">🌾 种植</button>' +
        ...
        qb.querySelector('#rw-qb-farm').onclick = function () {
