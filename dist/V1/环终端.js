...
(function () {
    ...
    function getMvuGlobal() {
        ...
        try { if (typeof HOST.Mvu !== 'undefined') return HOST.Mvu; } catch (e) {}
        return null;
    }
    function readState() {
        var m = getMvuGlobal();
...
`;
    ...
    function renderMapTab(st) {
        var html = '<div class="rw-card"><h3>🗺 局部地图（点击格子查看情报）</h3>' +
            ...
            '<div class="rw-map-hud"><span>种子 ' + esc(MapEngine.seed || '-') + '</span>' +
            '<span>缩放 <b id="rw-zoom-val">' + MapEngine.zoom.toFixed(1) + 'x</b></span>' +
    ...
    function bindMapTab() {
        ...
        canvas.addEventListener('mousemove', function (e) {
            ...
            var T = MapEngine.TILE * MapEngine.zoom;
            var x = Math.floor((e.clientX - r.left + MapEngine.camX) / T);
            var y = Math.floor((e.clientY - r.top + MapEngine.camY) / T);
            MapEngine.hover = { x: x, y: y };
            MapEngine.render();
        });
        canvas.addEventListener('mouseleave', function () { MapEngine.hover = null; MapEngine.render(); });
        canvas.addEventListener('click', function (e) {
            ...
            MapEngine.render();
            var info = MapEngine.tileInfo(x, y);
        ...
        if (zi) zi.onclick = function () { MapEngine.zoom = Math.min(3, MapEngine.zoom + 0.25); renderCurrent(); };
        if (zo) zo.onclick = function () { MapEngine.zoom = Math.max(0.6, MapEngine.zoom - 0.25); renderCurrent(); };
        if (rg) rg.onclick = function () { MapEngine.generate((Math.random() * 1e9) | 0); renderCurrent(); toast('地图已按新种子重生成', 'good'); };
    }
    ...
    function renderWorldTab(st) {
        ...
        html += '<div class="rw-card"><h3>🧠 AI 空间快照（注入给 AI 的语义摘要，实时生成）</h3>' +
            '<div style="font-size:11px;line-height:1.7;color:var(--rw-dim)">' + esc(MapEngine.snapshot()) + '</div></div>';
        return html;
    }
