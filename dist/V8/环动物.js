...
`;
    ...
    RW.husbandry = { open: function () { if (!readState()) { alert('[环世界] 状态未就绪：请先发送一条任意消息让系统初始化变量，再打开本面板。'); return; } try { if (HOST.Rimworld && HOST.Rimworld.closeAllPanels) HOST.Rimworld.closeAllPanels('rw-hus'); } catch (e) {} ensurePanel().classList.add('open'); render(); }, TAMABLE: TAMABLE };
